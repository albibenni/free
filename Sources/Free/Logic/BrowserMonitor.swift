...
class BrowserMonitor {
    ...
    private let browsers = ["com.google.Chrome", "com.apple.Safari", "com.brave.Browser", "com.microsoft.edgemac", "company.thebrowser.Browser", "com.operasoftware.Opera", "com.vivaldi.Vivaldi"]
    ...
    func checkActiveTab() {
        guard let appState = appState, appState.isBlocking, !appState.isPaused,
              ...
              let bundleId = frontApp.bundleIdentifier, browsers.contains(bundleId) else { return }

        if let lastRedirect = lastRedirectTime[bundleId], Date().timeIntervalSince(lastRedirect) < 2.0 { return }
    ...
    func getAllOpenUrls() -> [String] { automator.getAllOpenUrls(browsers: browsers) }
}
