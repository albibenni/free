...
struct CalendarImportService {
    ...
    static func mergedSchedulesWithImportedCalendarEvents(
        ...
        let importedSchedules = events
            ...
            .compactMap { event -> Schedule? in
                ...
                return Schedule(
                    id: existing?.id ?? deterministicImportedScheduleID(for: event.id),
                    name: event.title,
                    days: [],
    ...
    private static func deterministicImportedScheduleID(for eventID: String) -> UUID {
        if let parsed = UUID(uuidString: eventID) {
            return parsed
        }
