...
enum AppKitFluid {
    private static let sourceHTML = #"""
        ...
        <body>
          ...
          <script>
          // tiny '$.one' shim for the two jQuery-style calls used by the original pen code
          const $ = (selector) => {
            ...
            return {
              one: (eventName, handler) => {
                if (!element) return;
                const once = (event) => {
                  element.removeEventListener(eventName, once);
                  handler(event);
                };
                element.addEventListener(eventName, once, { passive: true });
              }
            };
          };
          </script>
    ...
    static let css = extractFirstMatch(
        pattern: #"<style>\s*([\s\S]*?)\s*</style>"#,
        in: sourceHTML
    )

    static let javascriptBlocks = extractAllMatches(
    ...
    private static func extractFirstMatch(pattern: String, in text: String) -> String {
