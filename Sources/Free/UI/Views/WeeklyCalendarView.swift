...
private final class WeeklyCalendarContainerNSView: NSView {
    ...
    override func layout() {
        ...
        let contentWidth = max(scrollView.contentSize.width, configuration.timeLabelWidth + configuration.timeColumnGutter + 7)
...
private final class WeeklyCalendarHeaderNSView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (index, day) in dayOrder.enumerated() {
            ...
            guard let date = weekRange.first(where: { calendar.component(.weekday, from: $0) == day }) else {
                continue
            }
...
private final class WeeklyCalendarDocumentNSView: NSView {
    ...
    private func drawSelectionPreview(configuration: WeeklyCalendarAppKitView) {
        ...
        let startHour = min(selectionStartPoint.y, selectionCurrentPoint.y) / configuration.hourHeight
        let endHour = max(selectionStartPoint.y, selectionCurrentPoint.y) / configuration.hourHeight
        let snappedStart = round(startHour * 4) / 4
        let snappedEnd = max(round(endHour * 4) / 4, snappedStart + 0.25)
        let rect = CGRect(
            x: calendarAreaX(configuration: configuration) + CGFloat(columnIndex) * dayColumnWidth(configuration: configuration) + 2,
    ...
    private func drawCurrentTimeIndicator(configuration: WeeklyCalendarAppKitView) {
        ...
        let x = calendarAreaX(configuration: configuration) + CGFloat(dayIndex) * dayColumnWidth(configuration: configuration)
...
private final class WeeklyCalendarScheduleBlockNSView: NSView {
    ...
    override func resetCursorRects() {
        ...
        if WeeklyCalendarView.canDirectlyManipulate(entry.schedule) {
            ...
            addCursorRect(CGRect(x: 0, y: 0, width: bounds.width, height: handleHeight), cursor: .resizeUpDown)
            addCursorRect(
                CGRect(
                    x: 0,
