...
struct AddScheduleView: View {
    ...
    init(
        ...
        let c = AddScheduleView.configure(
    ...
    static func configure(
        ...
        if let s = existingSchedule {
            return Configuration(
                name: s.name, days: s.days, isRecurring: s.date == nil, startTime: s.startTime,
                endTime: s.endTime, colorIndex: s.colorIndex, type: s.type, ruleSetId: s.ruleSetId)
        }
        let start =
        ...
        return Configuration(
            name: "", days: initialDay.map { [$0] } ?? [2, 3, 4, 5, 6], isRecurring: false,
            startTime: start, endTime: end, colorIndex: 0, type: .focus, ruleSetId: nil)
    }
    ...
    static func savePayload(
        ...
        guard
            let targetDate = Schedule.calculateOneOffDate(
                initialDay: initialDay, weekOffset: weekOffset,
