...
func createIcon(size: Int, scale: Int) {
    ...
    // Draw Background (Rounded Rectangle like macOS icons)
    let rect = NSRect(x: 0, y: 0, width: actualSize, height: actualSize)
    let path = NSBezierPath(roundedRect: rect.insetBy(dx: actualSize * 0.1, dy: actualSize * 0.1), xRadius: actualSize * 0.2, yRadius: actualSize * 0.2)
    ...
    if let symbol = NSImage(systemSymbolName: symbolName, accessibilityDescription: nil)?.withSymbolConfiguration(config) {
        let symbolRect = NSRect(x: actualSize * 0.25, y: actualSize * 0.25, width: actualSize * 0.5, height: actualSize * 0.5)
        NSColor.white.set()
